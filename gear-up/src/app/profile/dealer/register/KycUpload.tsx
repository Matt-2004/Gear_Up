...
const KycUpload: React.FC<KycUploadProps> = ({ documentType, onUploadFiles }) => {
    ...
    return (
        <div className="bg-gray-800 rounded-xl p-8 max-w-2xl w-full">
            <h3 className="text-2xl font-bold mb-3 text-white">Upload {getDocumentLabel()}</h3>
            <p className="text-gray-400 mb-6">
                Please upload clear photos of your {getDocumentLabel().toLowerCase()}. You can add multiple documents.
            </p>
                        ...
                        >
                            {item.preview ? (
                                ...
                                <label className="flex flex-col items-center cursor-pointer">
                                    ...
                                    <span className="text-gray-400 mb-1 text-sm">Click to upload</span>
            ...
            <div className="mt-6 p-4 bg-blue-900/30 border border-blue-700 rounded-lg">
                <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm text-blue-300">
                        {fileItems.filter((item) => item.file).length} / {fileItems.length} uploaded
                    </span>
                </div>
                <p className="text-xs text-blue-300">
                    Total size: {(
                        fileItems.reduce((sum, item) => sum + (item.file?.size || 0), 0) / 1024 / 1024
                    ).toFixed(2)} MB
                </p>
            </div>
