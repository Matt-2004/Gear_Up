...
export const StepNavigation = () => {
  ...
  const { isStepValid, isSubmitting, submitKycData, submitError } =
    useKycRegisterContext();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (step === Steps.length) {
      ...
      if (success) {
        ...
        addToastMessage(
          "error",
          submitError || "Failed to submit KYC. Please try again.",
        );
        setTimeout(() => {
          removeToastMessage();
        }, 2500);
      ...
      <div className="mt-8 w-full max-w-2xl">
        ...
        <div className="flex items-center justify-between gap-4">
          <button
            ...
            className={clsx(
              "rounded-xl px-6 py-3 font-medium transition-all duration-200 border-2",
              step === 1 || isSubmitting
